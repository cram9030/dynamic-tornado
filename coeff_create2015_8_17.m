function [results]=coeff_create2015_8_17(results,lattice,state,ref,geo,delta) 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
CX=results.FORCE(:,1,1)/(q*ref.S_ref);
CY=results.FORCE(:,1,2)/(q*ref.S_ref);
CZ=results.FORCE(:,1,3)/(q*ref.S_ref);
...
results.FORCE=squeeze(results.FORCE(1,1,:));
results.MOMENTS=squeeze(results.MOMENTS(1,1,:));


fac1=ref.b_ref /(2*state.U_inf);
fac2=ref.C_mac /(2*state.U_inf);

alpha_range = [state.alpha_root;state.alpha_root-delta;state.alpha_root+delta];

pCL = polyfit(alpha_range,CL,2);
pCD = polyfit(alpha_range,CD,2);
pCC = polyfit(alpha_range,CC,2);
pCX = polyfit(alpha_range,CX,2);
pCY = polyfit(alpha_range,CY,2);
pCZ = polyfit(alpha_range,CZ,2);
pCl = polyfit(alpha_range,Cl,2);
pCm = polyfit(alpha_range,Cm,2);
pCn = polyfit(alpha_range,Cn,2);

results.CL_a=polyval(pCL(1:2),state.alpha_root);
results.CD_a=polyval(pCD(1:2),state.alpha_root);
results.CC_a=polyval(pCC(1:2),state.alpha_root);
results.CX_a=polyval(pCX(1:2),state.alpha_root);
results.CY_a=polyval(pCY(1:2),state.alpha_root);
results.CZ_a=polyval(pCZ(1:2),state.alpha_root);
results.Cl_a=polyval(pCl(1:2),state.alpha_root);
results.Cm_a=polyval(pCm(1:2),state.alpha_root);
results.Cn_a=polyval(pCn(1:2),state.alpha_root);
