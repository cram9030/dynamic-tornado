function [masterState,masterLattice,masterGeo,masterResults,ref,forceTwist,CL,CD] = staticAlphaSweepBi6_19_2015(M,C,K,SegNum,halfWingLength,cord,centroid,ActLoc,cordNum,bw,bc,bh,noseH,zb,numSpanB,alphaRange,twistRange,beta,airSpeed,airDensity)
...
for j = twistRange
    for i = alphaRange
        ...
        masterLattice(j-min(twistRange)+1,i-min(alphaRange)+1) = generateLatticeBi6_19_2015(SegNum,Z,Phiz,X,Phix,Theta,cord,masterGeo(j-min(twistRange)+1,i-min(alphaRange)+1),cordNum,L,bw,bc,bh,noseH,zb,numSpanB,masterState(j-min(twistRange)+1,i-min(alphaRange)+1));
