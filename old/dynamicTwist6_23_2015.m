...
for t = twist(:,1)'
    ...
    alpha = alpha_root*ones(SegNum,1);
    ...
    artU_inf = [tempArtU_inf(1:SegNum/2);airSpeed*ones(2*numSpanB,1);tempArtU_inf(SegNum/2+1:end)];
    tempAlpha_aero = atan((ActLoc(1)^2*[flipud(staticVel(5:5:end));staticVel(5:5:end)]+airSpeed*sin(alpha))./(airSpeed*cos(alpha)));
    alpha_aero = [tempAlpha_aero(1:SegNum/2);alpha_root*ones(2*numSpanB,1);tempAlpha_aero(SegNum/2+1:end)];
