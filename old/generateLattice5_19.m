%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [lattice] = generateLattice5_19(SegNum,Z,Phiz,X,Phix,Theta,PhiTheta,cord,centroid,cordNum,L,bw,bc,noseH,zb,numSpanB,state)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = numSpanB/4+1:3*numSpanB/4
...
hyp = sqrt((noseCord*bc/cordNum)^2+(bh/4)^2);
sinn = -bh/(4*hyp);
cosn = noseCord*bc/(cordNum*hyp);
slopeR = [cosn,0,sinn;0,1,0;-sinn,0,cosn];

ymbNose = bw/numSpanB*[numSpanB/4+.5:3*numSpanB/4-.5]+sum(L)/2;
y1nNose = bw/numSpanB*[numSpanB/4:3*numSpanB/4-1]+sum(L)/2;
y2nNose = bw/numSpanB*[numSpanB/4+1:3*numSpanB/4]+sum(L)/2;
...
for i = 0:pi:pi
    ...
    noseRelmb = rotX*[noseRelmb(1,:);noseRelmb(2,:)-(sum(L)+bw)/2;noseRelmb(3,:)+bh/4];
    noseRel1n = rotX*[noseRel1n(1,:);noseRel1n(2,:)-(sum(L)+bw)/2;noseRel1n(3,:)+bh/4];
    noseRel2n = rotX*[noseRel2n(1,:);noseRel2n(2,:)-(sum(L)+bw)/2;noseRel2n(3,:)+bh/4];
    noseReLeadL = rotX*[noseReLeadL(1,:);noseReLeadL(2,:)-(sum(L)+bw)/2;noseReLeadL(3,:)+bh/4];
    noseReLeadR = rotX*[noseReLeadR(1,:);noseReLeadR(2,:)-(sum(L)+bw)/2;noseReLeadR(3,:)+bh/4];
    noseReTrailL = rotX*[noseReTrailL(1,:);noseReTrailL(2,:)-(sum(L)+bw)/2;noseReTrailL(3,:)+bh/4];
    noseReTrailR = rotX*[noseReTrailR(1,:);noseReTrailR(2,:)-(sum(L)+bw)/2;noseReTrailR(3,:)+bh/4];
...
hyp = sqrt((noseCord*bc/cordNum)^2+(bw/4)^2);
sinn = bw/(4*hyp);
cosn = noseCord*bc/(cordNum*hyp);
...
%     
%     noseRelmb = rotX*[noseRelmb(1,:);noseRelmb(2,:)+bw/4;noseRelmb(3,:)];
%     noseRel1n = rotX*[noseRel1n(1,:);noseRel1n(2,:)+bw/4;noseRel1n(3,:)];
%     noseRel2n = rotX*[noseRel2n(1,:);noseRel2n(2,:)+bw/4;noseRel2n(3,:)];
%     noseReLeadL = rotX*[noseReLeadL(1,:);noseReLeadL(2,:)+bw/4;noseReLeadL(3,:)];
%     noseReLeadR = rotX*[noseReLeadR(1,:);noseReLeadR(2,:)+bw/4;noseReLeadR(3,:)];
%     noseReTrailL = rotX*[noseReTrailL(1,:);noseReTrailL(2,:)+bw/4;noseReTrailL(3,:)];
%     noseReTrailR = rotX*[noseReTrailR(1,:);noseReTrailR(2,:)+bw/4;noseReTrailR(3,:)];
% 
%     xmb = [xmb;noseRelmb(1,:)'-noseH-centroid(1)];
%     ymb = [ymb;noseRelmb(2,:)'+(sum(L)+bw)/2];
%     zmb = [zmb;noseRelmb(3,:)'+zb];
...
% 
%     SNose = [SNose,pi*ones(1,length(y1nNose)*length(xTrailNose))];
%     DiHiAng = [DiHiAng;0*ones(length(y1nNose)*length(xTrailNose),1)];
% end
