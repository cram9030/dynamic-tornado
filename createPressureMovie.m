%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [] = createPressureMovie(lattice,results,Q,filename)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
rotate3d on 
colormap(hot);
fill3(lattice(1).XYZ(:,:,1)',lattice(1).XYZ(:,:,2)',lattice(1).XYZ(:,:,3)',results(1).cp'*Q(1))
...
caxis([min(min([results(:).cp]'))*min(Q) max(max([results(:).cp]'))*max(Q)])
...
for j = 1:N
    fill3(lattice(j).XYZ(:,:,1)',lattice(j).XYZ(:,:,2)',lattice(j).XYZ(:,:,3)',results(j).cp'*Q(j))
    drawnow
    F(j) = getframe(gcf);
end
