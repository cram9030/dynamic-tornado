...
usrfun = 'dynamicTwist2016_2_21';

% setup the snopt input parameters
t = [0:dt:tfin]';
x_init = [pi/180*sin(2*pi*5*t);pi/180*sin(2*pi*5*t)]; %intialize to sine and cos at 5 hertz frequency
...
Flow =[      0; 
             ...
             0;
             0;
             (weight-weight*0.1)*ones(nnObj/2-1,1);
             zeros(nnObj/2-1,1);
             zeros(nnObj/2-1,1)]; 
...
[x_opt,F,xmul,Fmul,inform] = snopt( x_init,xlow,xupp,xmul,xstate, ...
				    ...
				    @dynamicTwist2016_2_21,ObjAdd,ObjRow,A,iAfun,jAvar, ...
				    iGfun,jGvar);
