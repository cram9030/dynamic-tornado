function [results]=coeff_create6_24_2015(results,lattice,state,ref,geo) 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
tempAlpha = [];
for i = 1:geo.nx
    tempAlpha=[tempAlpha;state.alpha];
end
...
for i=1:lemma(1)
    ...
    B2WTransform=[cos(state.betha)*cos(tempAlpha(i)),        -sin(state.betha),          cos(state.betha)*sin(tempAlpha(i)) ;...
              cos(tempAlpha(i))*sin(state.betha),         cos(state.betha),          sin(state.betha)*sin(tempAlpha(i)) ;...
                              -sin(tempAlpha(i)),                        0,                           cos(tempAlpha(i))];
...
cord = zeros(geo.numSpanW,1);
tempForce = zeros(geo.numSpanW,1);
shearforce = zeros(geo.numSpanW,1);
bendingmoment = zeros(geo.numSpanW,1);
...
temp = [lattice.COLLOC(1:geo.numSpanW,2),results.ForcePerMeter,tempForce,results.CL_local];
temp = sortrows(temp);

shearforce(1:(geo.numSpanW)/2) = cumsum(temp(1:(geo.numSpanW)/2,3));
shearforce((geo.numSpanW)/2+1:end) = -flipud(cumsum(flipud(temp((geo.numSpanW)/2+1:end,3))));


bendingmoment(1:(geo.numSpanW)/2) = cumsum(temp(1:(geo.numSpanW)/2,3).*temp(1:(geo.numSpanW)/2,1));
bendingmoment((geo.numSpanW)/2+1:end) = flipud(cumsum(-flipud(temp((geo.numSpanW)/2+1:end,3)).*flipud(temp((geo.numSpanW)/2+1:end,1))));
