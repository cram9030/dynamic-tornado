function [results]=coeff_create5_7_2015(results,lattice,state,ref,geo) 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
npan = length(lattice.COLLOC); %Number of panels per wing, only valid if there is only one wing
...
noofpanels = length(lattice.COLLOC); %number of panels, only valid for a single wing
...
cord = zeros(geo.numSpanW+geo.numSpanB,1);
tempForce = zeros(geo.numSpanW+geo.numSpanB,1);
shearforce = zeros(geo.numSpanW+geo.numSpanB,1);
bendingmoment = zeros(geo.numSpanW+geo.numSpanB,1);

for i = 1:geo.numSpanW
    results.ForcePerMeter(i,1) = sum(FPM(i:geo.numSpanW:geo.nx*geo.numSpanW));
    cord(i) = max(max(lattice.XYZ(i:geo.numSpanW:geo.nx*geo.numSpanW,:,1)))-min(min(lattice.XYZ(i:geo.numSpanW:geo.nx*geo.numSpanW,:,1)));
    tempForce(i) = sum(forceMagn(i:geo.numSpanW:geo.nx*geo.numSpanW));
    results.CL_local(i,1) = sum(LPM(i:geo.numSpanW:geo.nx*geo.numSpanW));
end
for j = i+1:i+geo.numSpanB
    results.ForcePerMeter(j,1) = sum(FPM(geo.numSpanW*geo.nx+j-geo.numSpanW:geo.numSpanB:end));
    cord(j) = max(max(lattice.XYZ(geo.numSpanW*geo.nx+j-geo.numSpanW:geo.numSpanB:end,:,1)))-min(min(lattice.XYZ(geo.numSpanW*geo.nx+j-geo.numSpanW:geo.numSpanB:end,:,1)));
    tempForce(j) = sum(forceMagn(geo.numSpanW*geo.nx+j-geo.numSpanW:geo.numSpanB:end));
    results.CL_local(j,1) = sum(LPM(geo.numSpanW*geo.nx+j-geo.numSpanW:geo.numSpanB:end));
end

temp = [[lattice.COLLOC(1:geo.numSpanW,2);lattice.COLLOC(end-geo.numSpanB+1:end,2)],results.ForcePerMeter,tempForce,results.CL_local];
temp = sortrows(temp);

shearforce(1:(geo.numSpanW+geo.numSpanB)/2) = cumsum(temp(1:(geo.numSpanW+geo.numSpanB)/2,3));
shearforce((geo.numSpanW+geo.numSpanB)/2+1:end) = -flipud(cumsum(flipud(temp((geo.numSpanW+geo.numSpanB)/2+1:end,3))));


bendingmoment(1:(geo.numSpanW+geo.numSpanB)/2) = cumsum(temp(1:(geo.numSpanW+geo.numSpanB)/2,3).*temp(1:(geo.numSpanW+geo.numSpanB)/2,1));
bendingmoment((geo.numSpanW+geo.numSpanB)/2+1:end) = flipud(cumsum(-flipud(temp((geo.numSpanW+geo.numSpanB)/2+1:end,3)).*flipud(temp((geo.numSpanW+geo.numSpanB)/2+1:end,1))));

results.ystation = temp(:,1);
results.ForcePerMeter = temp(:,2);
results.ShearForce = shearforce;
results.BendingMoment = bendingmoment;
results.CL_local = temp(:,3);
