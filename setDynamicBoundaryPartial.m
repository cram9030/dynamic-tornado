function [bc,bci,BC_alpha]=setDynamicBoundaryPartial(lattice,state,geo,h)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:a
    e = zeros(a,1);
    e(i) = 1j;
    Wind(:,:,i)=([V.*cos(state.alpha+e).*cos(state.betha) -V.*cos(state.alpha+e).*sin(state.betha) V.*sin(state.alpha+e)]);   
end
    
BC_alpha=sum(lattice.N.*Wind,2)';  %angle of attack derivative
%%%%%%%%%%%%%%%%%%%%%%%
